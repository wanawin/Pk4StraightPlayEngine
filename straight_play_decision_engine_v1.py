...
# Reusable AABC straight-play companion app with live playlist + seed/member operational replay audit + ZLT secondary selector visibility
...
BUILD_ID = "STRAIGHT_DECISION_ENGINE_V11_PRINTABLE_PLAYLIST_RUN_BUTTON_SEED_FIX__2026-06-15"
...
def parse_printable_playlist_text(text: str) -> pd.DataFrame:
    ...
    Supports BOTH compact formats:
      # STREAM MEMBER SEED FIT
      01 Connecticut Play 4 Day            0255    9436  26.57

      # STREAM CORE MEMBER SEED FIT
      01 Wisconsin Pick 4 Evening          025     0255  6467 31.66

    The parser reads PLAY_DATE/HISTORY_THROUGH/CORE once from the header and
    keeps each row printable while still giving the straight engine stream,
    member, seed, fit_score, core, and play_date columns.
    """
    ...
    for ln in lines[:30]:
        ...
        mhist = re.search(r"HISTORY[_ ]?THROUGH\s*[:=]\s*([0-9]{4}[-/][0-9]{1,2}[-/][0-9]{1,2})", ln, re.I)
        if mhist:
            history_through = pd.to_datetime(mhist.group(1), errors="coerce")
        mcore = re.search(r"^\s*CORE\s*[:=]\s*(\d{3})\s*$", ln, re.I)
        if mcore:
            header_core = mcore.group(1)

    compact_rows = []
    rownum_re = re.compile(r"^\s*(?P<rank>\d{1,3})[\.)]?\s+(?P<body>.+?)\s*$")
    # Parse from the RIGHT so streams can contain spaces, punctuation, parentheses, ellipsis, etc.
    # Required right-side tokens are member + seed + fit. Optional core may appear before member.
    right_re = re.compile(
        r"^(?P<stream>.*?)\s+"
        r"(?:(?P<core>\d{3})\s+)?"
        r"(?P<member>\d{4})\s+"
        r"(?P<seed>\d{4}|\d[- ]\d[- ]\d[- ]\d)\s+"
        r"(?P<fit>-?\d+(?:\.\d+)?)\s*$"
    )
    for ln in lines:
        raw = ln.strip()
        if not raw or raw.startswith(('#', '-', '=')):
            continue
        m0 = rownum_re.match(raw)
        if not m0:
            continue
        m = right_re.match(m0.group('body'))
        if not m:
            continue
        ...
        seed = norm4(m.group('seed'))
        if not member or not is_aabc_member(member) or not seed:
            continue
        row = {
            'playlist_rank': int(m0.group('rank')),
            'stream': stream,
            'state': stream,
            'game': '',
            'core': m.group('core') or header_core or family_from_member(member),
            'member': member,
            'seed': seed,
        ...
        if history_through is not None and not pd.isna(history_through):
            row['history_through'] = history_through.date().isoformat()
...
def zlt_select_single_from_gate(member: str, seed: str, rules: pd.DataFrame, gate_candidates: Sequence[str], freq_order: Sequence[str]) -> dict:
    """Secondary/ZLT selector inside the primary gate.

    The primary gate may contain 1-3 permutations. This function does not change
    the gate; it chooses the final single from inside that gate by looking for
    exact-permutation rules already present in the uploaded rule database.

    This also preserves/read-throughs any extra rule DB columns by storing the
    selected fired row's metadata in the output. If no exact-permutation rule
    fires, the function falls back to the existing frequency/first-candidate
    behavior and labels that explicitly.
    """
    member = norm_member(member) or str(member).zfill(4)
    seed = norm4(seed) or str(seed).zfill(4)
    cands = [str(c).strip() for c in (gate_candidates or []) if str(c).strip()]
    if not cands:
        return {
            "zlt_status": "NO_GATE_CANDIDATES",
            "zlt_single": "",
            "zlt_rule": "",
            "zlt_rule_score": 0.0,
            "zlt_rule_count": 0,
            "zlt_candidate_scores": "",
            "zlt_source": "NONE",
        }
    atoms = atom_set_for_seed(seed, member)
    if rules is None or rules.empty:
        fallback = sorted(cands, key=lambda p: (list(freq_order).index(p) if p in list(freq_order) else 999, p))[0]
        return {
            "zlt_status": "NO_RULE_DB_GATE_FREQ_FALLBACK",
            "zlt_single": fallback,
            "zlt_rule": "FREQ_FALLBACK_WITHIN_GATE",
            "zlt_rule_score": 0.0,
            "zlt_rule_count": 0,
            "zlt_candidate_scores": "",
            "zlt_source": "FREQ_FALLBACK",
        }
    ...
    if sub.empty:
        fallback = sorted(cands, key=lambda p: (list(freq_order).index(p) if p in list(freq_order) else 999, p))[0]
        return {
            "zlt_status": "NO_MEMBER_RULES_GATE_FREQ_FALLBACK",
            "zlt_single": fallback,
            "zlt_rule": "FREQ_FALLBACK_WITHIN_GATE",
            "zlt_rule_score": 0.0,
            "zlt_rule_count": 0,
    ...
    fired_by_perm = []
    for perm in cands:
        gsub = sub[sub["gate"].astype(str).eq(str(perm))].copy()
        matched = []
        for _, rr in gsub.iterrows():
            atom_str = str(rr.get("atoms_str", ""))
            req = [a.strip() for a in atom_str.split("&") if a.strip()]
            if req and all(a in atoms for a in req):
                matched.append(rr.to_dict())
        if matched:
            mdf = pd.DataFrame(matched)
            mdf["rule_score"] = pd.to_numeric(mdf.get("rule_score", 0), errors="coerce").fillna(0)
            mdf["train_pos"] = pd.to_numeric(mdf.get("train_pos", 0), errors="coerce").fillna(0)
            mdf["train_out"] = pd.to_numeric(mdf.get("train_out", 0), errors="coerce").fillna(0)
            best = mdf.sort_values(["rule_score", "train_pos", "train_out"], ascending=[False, False, True]).iloc[0].to_dict()
            scores = sorted(pd.to_numeric(mdf["rule_score"], errors="coerce").fillna(0).tolist(), reverse=True)
            final_score = float(best.get("rule_score", 0)) * 1.7 + sum(scores[:5]) * 0.18 + float(best.get("train_pos", 0)) * 0.03 - float(best.get("train_out", 0)) * 0.15
            fired_by_perm.append({
                "perm": perm,
                "score": final_score,
                "rule_count": int(len(mdf)),
                "best_rule": str(best.get("atoms_str", "")),
                "best_rule_score": float(best.get("rule_score", 0)),
                "best_train_pos": int(float(best.get("train_pos", 0) or 0)),
                "best_train_out": int(float(best.get("train_out", 0) or 0)),
                "best_rule_source": str(best.get("rule_source", "")),
            })
    if fired_by_perm:
        # Tie-breaker stays inside the gate: ZLT score, rule count, frequency order, lexical.
        freq_list = list(freq_order)
        fired_by_perm = sorted(fired_by_perm, key=lambda d: (d["score"], d["rule_count"], -(freq_list.index(d["perm"]) if d["perm"] in freq_list else 999), d["perm"]), reverse=True)
        best = fired_by_perm[0]
        return {
            "zlt_status": "ZLT_RULE_FIRED",
            "zlt_single": best["perm"],
            "zlt_rule": best["best_rule"],
            "zlt_rule_score": round(float(best["best_rule_score"]), 4),
            "zlt_rule_count": int(best["rule_count"]),
            "zlt_candidate_scores": " | ".join([f"{d['perm']}:{round(d['score'],4)}({d['rule_count']})" for d in fired_by_perm]),
            "zlt_source": best.get("best_rule_source", ""),
        }
    fallback = sorted(cands, key=lambda p: (list(freq_order).index(p) if p in list(freq_order) else 999, p))[0]
    return {
        "zlt_status": "NO_ZLT_EXACT_RULE_FIRED_GATE_FREQ_FALLBACK",
        "zlt_single": fallback,
        "zlt_rule": "FREQ_FALLBACK_WITHIN_GATE",
        "zlt_rule_score": 0.0,
        "zlt_rule_count": 0,
...
if playlist_file is None:
    ...
    try:
        ...
        st.markdown("### Playlist diagnostics")
        st.write("Detected playlist columns:", list(playlist.columns))
        ...
        d1.metric("Playlist rows parsed", len(playlist))
        d2.metric("Seed column", auto_seed or "NOT FOUND")
        d3.metric("Member column", auto_member or "NOT FOUND")
        d4.metric("Stream column", auto_stream or "NOT FOUND")
        ...
        show_fired_detail = st.checkbox("Show fired-rule detail", value=False)

        run_straight = st.button("RUN STRAIGHT ENGINE", type="primary", use_container_width=True)
        if not run_straight and "straight_out" not in st.session_state:
            st.info("Check the detected columns above, then click RUN STRAIGHT ENGINE.")
        if run_straight:
            output_rows = []
            fired_all = []
            for idx, row in playlist.iterrows():
                ...
                zlt = zlt_select_single_from_gate(member, seed, rule_db, top1_cands, freq_order) if not rule_db.empty else {
                    "zlt_status": "NO_RULE_DB_GATE_FREQ_FALLBACK",
                    "zlt_single": top1_cands[0] if top1_cands else "",
                    "zlt_rule": "FREQ_FALLBACK_WITHIN_GATE",
                    "zlt_rule_score": 0,
                    "zlt_rule_count": 0,
                    "zlt_candidate_scores": "",
                ...
                single = zlt.get("zlt_single", "") or (sorted(top1_cands, key=lambda p: (freq_order.index(p) if p in freq_order else 999, p))[0] if top1_cands else "")

                rec = {
                    "#": int(row.get("playlist_rank", idx + 1)) if str(row.get("playlist_rank", idx + 1)).isdigit() else idx + 1,
                    "stream": stream,
                    "play_date": str(play_date.date()) if play_date is not None and not pd.isna(play_date) else str(row.get("play_date", "")),
                    "member": member,
                    "family": family_from_member(member),
                    "seed": seed,
                    "fit_score": row.get("fit_score", ""),
                    "status": "OK_RULES_FIRED" if fired is not None and not fired.empty else ("NO RULE DB - FREQ FALLBACK" if rule_db.empty else "NO_RULE_FIRED_FREQ_FALLBACK"),
                    ...
                    "zlt_status": zlt.get("zlt_status", ""),
                    "zlt_rule": zlt.get("zlt_rule", ""),
                    "zlt_rule_count": zlt.get("zlt_rule_count", 0),
                }
            ...
            if not out.empty:
                printable_lines = []
                play_date_val = ""
                if "play_date" in out.columns:
                    vals = [v for v in out["play_date"].astype(str).unique().tolist() if v and v.lower() != "nan"]
                    play_date_val = vals[0] if vals else ""
                printable_lines.append(f"STRAIGHT PLAYLIST | PLAY_DATE: {play_date_val}")
                printable_lines.append(f"BUILD: {BUILD_ID}")
                printable_lines.append("")
                printable_lines.append("#  STREAM                           MBR    SEED   SINGLE  TOP1 GATE")
                printable_lines.append("-" * 86)
                for _, r in out.iterrows():
                    stream_txt = str(r.get("stream", ""))[:31]
                    printable_lines.append(
                        f"{int(r.get('#', 0)):02d} {stream_txt:<31} {str(r.get('member','')):<6} {str(r.get('seed','')):<6} {str(r.get('single_straight_candidate','')):<7} {str(r.get('top1_candidates',''))}"
                    )
                printable_txt = "\n".join(printable_lines)

            st.session_state["straight_out"] = out
            st.session_state["straight_csv"] = out.to_csv(index=False).encode("utf-8") if not out.empty else b""
            st.session_state["straight_printable_txt"] = printable_txt
            st.session_state["straight_fired_df"] = pd.concat(fired_all, ignore_index=True) if fired_all else pd.DataFrame()

        # Persist results after button/download-triggered reruns.
        if "straight_out" in st.session_state and isinstance(st.session_state["straight_out"], pd.DataFrame):
            out = st.session_state["straight_out"]
            st.markdown("### Straight recommendations")
            st.dataframe(out, use_container_width=True)

            if not out.empty:
                st.markdown("### Printable view")
                st.text(st.session_state.get("straight_printable_txt", ""))
                c1, c2 = st.columns(2)
                c1.download_button(
                    "Download straight recommendations CSV",
                    st.session_state.get("straight_csv", b""),
                    file_name="straight_play_recommendations.csv",
                    mime="text/csv",
                    key="download_straight_csv_persisted",
                )
                c2.download_button(
                    "Download printable straight TXT",
                    st.session_state.get("straight_printable_txt", "").encode("utf-8"),
                    file_name="straight_play_recommendations_PRINTABLE.txt",
                    mime="text/plain",
                    key="download_straight_txt_persisted",
                )

            fired_df = st.session_state.get("straight_fired_df", pd.DataFrame())
            if isinstance(fired_df, pd.DataFrame) and not fired_df.empty:
                if show_fired_detail:
                    st.write("Fired rule detail")
                    st.dataframe(fired_df.head(500), use_container_width=True)
                st.download_button(
                    "Download fired rule detail CSV",
                    fired_df.to_csv(index=False).encode("utf-8"),
                    file_name="straight_fired_rule_detail.csv",
                    mime="text/csv",
                    key="download_fired_detail_persisted",
                )
    except Exception as e:
...
def run_seed_member_operational_audit(audit_df: pd.DataFrame, rules: pd.DataFrame, hist: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for _, row in audit_df.iterrows():
        ...
        top2_cands = candidates_from_top_gates(scored, 2) if scored else freq_order[:6]
        top3_cands = candidates_from_top_gates(scored, 3) if scored else freq_order[:9]
        zlt = zlt_select_single_from_gate(member, seed, rules, top1_cands, freq_order)
        single = zlt.get("zlt_single", "") or (sorted(top1_cands, key=lambda p: (freq_order.index(p) if p in freq_order else 999, p))[0] if top1_cands else "")
        ...
        rows.append({
            **base,
            ...
            "top1_score": round(scored[0]["score"], 4) if scored else 0,
            "zlt_status": zlt.get("zlt_status", ""),
            "zlt_single": zlt.get("zlt_single", ""),
            "zlt_rule": zlt.get("zlt_rule", ""),
            "zlt_rule_score": zlt.get("zlt_rule_score", 0),
            "zlt_rule_count": zlt.get("zlt_rule_count", 0),
            "zlt_candidate_scores": zlt.get("zlt_candidate_scores", ""),
            "zlt_source": zlt.get("zlt_source", ""),
...
def make_audit_txt_report(result: pd.DataFrame, summary: pd.DataFrame) -> str:
    ...
    lines.append("v10 includes ZLT/secondary selector visibility inside the Top1 gate when exact-permutation rules are present.")
    lines.append("")
    ...
    if result is None or result.empty:
        ...
        for _, r in result.iterrows():
            ...
            lines.append(f"   Primary Rule: {r.get('top1_best_rule','')}")
        lines.append(f"   ZLT: {r.get('zlt_status','')} | Single={r.get('zlt_single','')} | Rule={r.get('zlt_rule','')}")
    return "\n".join(lines)
