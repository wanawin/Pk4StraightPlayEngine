...
# Build marker: STRAIGHT_DECISION_ENGINE_V8_SEED_MEMBER_AUDIT__2026-06-14
...
BUILD_ID = "STRAIGHT_DECISION_ENGINE_V8_SEED_MEMBER_AUDIT__2026-06-14"
...
def run_seed_member_operational_audit(audit_df: pd.DataFrame, rules: pd.DataFrame, hist: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for _, row in audit_df.iterrows():
        ...
        if fired is not None and not fired.empty:
            tmp = fired.copy()
            tmp.insert(0, "audit_id", row.get("audit_id", ""))
            tmp.insert(1, "member", member)
            tmp.insert(2, "seed", seed)
            tmp.insert(3, "stream", row.get("stream", ""))
            tmp.insert(4, "event_date", row.get("event_date", ""))
